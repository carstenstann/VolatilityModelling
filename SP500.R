...
RV.na <- import %>% select(SPX2.rv) %>% 
                 as.xts(order.by = import$DateID) ##Select index for analysis here FTSE2.rv, N2252.rv, GDAXI2.rv,DJI2.rv, FCHI2.rv

returns.na <- import %>% select(SPX2.r) %>% 
  as.xts(order.by = import$DateID) ##select index here: FTSE2.r, N2252.r, GDAXI2.r,DJI2.r, FCHI2.r
...
## fix column names
colnames(RV) <- c("SP500") ##"FTSE100", "Nikkei225", "DAX", "DJI", "CAC40"
colnames(returns) <- c("SP500") ##"FTSE100", "Nikkei225", "DAX", "DJI", "CAC40"
...
Acf.returns.SP500.c <- Acf(crisis.r^2, lag.max = 50) %>% tidy()
...
##Threshold ARCH (TARCH Glosten et al (1993)) aka GJR GARCH in rugarch
## EGARCH Nelson 1991
## APARCH Ding et al 1993 "apARCH" in rugarch sGARCH (resutled in same exact loss as GARCH11 - leave out from now on)
...
forecast.1 <- as.xts(var.mat[,1:12], order.by = index(crisis.rv$SP500))
names(forecast.1)<- c("ARCH1", "ARCH2", "ARCH3", "ARCH4", "ARCH5", 
                      "TARCH1", "TARCH2", "TARCH3", "TARCH4", "TARCH5", 
                      "GARCH11", "TGARCH11")


plot(forecast.1$TGARCH11, ylim = c(0,0.008) )
...
plots <- c(12,11,9,10)
models <- c("TGARCH(1,1)","GARCH(1,1)", "TARCH(4)", "TARCH(5)")


vol.forecast<-as.xts((roll.list[[12]]@forecast$density[,"Sigma"])^2, 
                       order.by = index(crisis.rv))
vol.plot<- cbind(crisis.rv,vol.forecast)
names(vol.plot)<- c("RV", "Volatility Forecast")
plot.zoo(vol.plot, col = 1:2, ylab = "", xlab = "Date",screens = 1, lty = c(1:2), lwd = c(1.2,2))
legend("topright", legend = names(vol.plot),col=1:2, lty=1,cex=0.85) 
title(main = "S&P 500 - TGARCH(1,1)")

vol.forecast<-as.xts((roll.list[[11]]@forecast$density[,"Sigma"])^2, 
                     order.by = index(crisis.rv))
vol.plot<- cbind(crisis.rv,vol.forecast)
names(vol.plot)<- c("RV", "Volatility Forecast")
plot.zoo(vol.plot, col = 1:2, ylab = "", xlab = "Date",screens = 1, lty = c(1:2), lwd = c(1.2,2))
legend("topright", legend = names(vol.plot),col=1:2, lty=1,cex=0.85) 
title(main = "S&P 500 - GARCH(1,1)")

vol.forecast<-as.xts((roll.list[[8]]@forecast$density[,"Sigma"])^2, 
                     order.by = index(crisis.rv))
vol.plot<- cbind(crisis.rv,vol.forecast)
names(vol.plot)<- c("RV", "Volatility Forecast")
plot.zoo(vol.plot, col = 1:2, ylab = "", xlab = "Date",screens = 1, lty = c(1:2), lwd = c(1.2,2))
legend("topright", legend = names(vol.plot),col=1:2, lty=1,cex=0.85) 
title(main = "S&P 500 - TARCH(3)")

vol.forecast<-as.xts((roll.list[[9]]@forecast$density[,"Sigma"])^2, 
                     order.by = index(crisis.rv))
vol.plot<- cbind(crisis.rv,vol.forecast)
names(vol.plot)<- c("RV", "Volatility Forecast")
plot.zoo(vol.plot, col = 1:2, ylab = "", xlab = "Date",screens = 1, lty = c(1:2), lwd = c(1.2,2))
legend("topright", legend = names(vol.plot),col=1:2, lty=1,cex=0.85) 
title(main = "S&P 500 - TARCH(4)")
