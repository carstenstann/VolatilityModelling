...
First, a set of ARCH models, developed by Engle (1982), are tested. These models are similar to the GARCH specification presented below when p = 0. This analysis tests q = {1, 2, 3} for the ARCH(q) specification. 

Secondly, the GARCH(p,q) model of Bollerslev (1986) is tested with p and q both equal to 1. 

The previous specifications assume the response to shocks are independent of the sign of the shock (i.e. positive and negative shocks of equal size have equal impact) and are merely a function of shock size. Several GARCH model extensions seek to accomodate asymmetry in the response variable. One such model, proposed by Zakoian (1994), models conditional standard deviation rather than conditional variance, as specificed below. This analysis first considers Threshold ARCH(q) specifications (p = 0) of q = {1, 2, 3} as well as a TGARCH(1, 1) specification. 
